...
import { useSupplierSelection } from "../hooks/use-supplier-selection";
  ...
  const onSubmit = async (values: AddMaterialFormValues) => {
    try {
      ...
      if (imageUpload.imageFile) {
        const uploadedUrl = await storageService.uploadMaterialImage(
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                У вас есть несохраненные изменения. Все заполненные данные будут
                потеряны.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>

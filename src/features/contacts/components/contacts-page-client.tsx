...
import { Company, Contact, CompanyType, ContactType } from "@/types";
  ...
  const handleAddCompany = async (
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Если указан представитель - создаем контакт
        if (representative && representative.name) {
          const contactPayload: Omit<
            Contact,
          ...
          > = {
            ...
            // Сопоставляем тип компании и контакта
            type:
              company.type === CompanyType.CLIENT
                ? ContactType.CLIENT
                : ContactType.SUPPLIER,
            address: "",
            notes: `Основной представитель компании ${company.name}`,
          };

          const contactResult = await createContact(contactPayload);
          if (contactResult.success) {
            toast.success(`Представитель ${representative.name} добавлен`);
            await loadContacts();
          } else {
            console.error(
            ...
            toast.error(
              "Компания добавлена, но не удалось создать контакт представителя",
            );
          }
        }
      ...
      toast.error("Произошла ошибка при добавлении компании");
    }
  };
